...
const logPath = path.join(appData, 'SimHub', 'WhatsAppPlugin', 'logs', 'baileys.log');
        ...
        try {
            ...
            if (data.type === 'sendReply') {
                ...
                send({
                    ...
                    error: 'Chat contacts list is not supported with Baileys backend. Please use whatsapp-web.js.'
                });
            }
        } catch (err) {
    ...
    try {
        ...
        sock.ev.on('connection.update', async (update) => {
            ...
            if (connection === 'close') {
                ...
                log('[BAILEYS] Connection closed. Reason: ' + (lastDisconnect?.error?.message || 'Unknown'));
