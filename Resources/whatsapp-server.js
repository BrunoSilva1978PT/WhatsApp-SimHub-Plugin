...
const logPath = path.join(appData, "SimHub", "WhatsAppPlugin", "logs", "node.log");
...
process.on("uncaughtException", (err) => {
    ...
    try {
        ...
        try {
            fs.appendFileSync(logPath, "[FATAL] Error in error handler: " + e.message + "\n");
        } catch (e2) {
                ...
                try {
                    ...
                    const result = await client.pupPage.evaluate(async (chatId, messageText) => {
                        try {
                            ...
                            const message = await window.WWebJS.sendMessage(chat, messageText, {}, 0);
    ...
    try {
        ...
        if (readyTimestamp > 0 && msg.timestamp * 1000 < readyTimestamp) {
            ...
            if (oldMessagesIgnoredCount <= 20) {
                log("[MSG] IGNORED - message too old (before ready) [" + oldMessagesIgnoredCount + "]");
            } else if (oldMessagesIgnoredCount === 21) {
                log("[MSG] IGNORED - stopping logs (too many old messages, " + oldMessagesIgnoredCount + "+ total)");
            }
            return;
        }
        ...
        try {
            ...
            if (msg.id.participant) log("[DEBUG] msg.id.participant: " + msg.id.participant);
        ...
        try {
            if (msg._data) {
                ...
                if (msg._data.sender) {
                    ...
                    if (msg._data.sender.id) log("[DEBUG] msg._data.sender.id: " + JSON.stringify(msg._data.sender.id));
                    if (msg._data.sender.user) log("[DEBUG] msg._data.sender.user: " + msg._data.sender.user);
                    if (msg._data.sender.number) log("[DEBUG] msg._data.sender.number: " + msg._data.sender.number);
                }
                
                // Verificar participant
                if (msg._data.participant) {
                    log("[DEBUG] msg._data.participant FOUND!");
                    log("[DEBUG] msg._data.participant: " + JSON.stringify(msg._data.participant));
                }
            }
        } catch (e) {
        ...
        try {
            ...
            if (contact) {
                ...
                if (contact.id) {
                    ...
                    if (contact.id.user) log("[DEBUG] contact.id.user: " + contact.id.user);
                    if (contact.id._serialized) log("[DEBUG] contact.id._serialized: " + contact.id._serialized);
                }
                if (contact.number) log("[DEBUG] contact.number: " + contact.number);
                if (contact.pushname) log("[DEBUG] contact.pushname: " + contact.pushname);
        ...
        // ⚠️ MÉTODO 2-4: Se é @lid, tentar obter número real
            ...
            if (!number) {
                ...
                log("[MSG]    1. Save contact in your phone with full number (+351...)");
...
client.on("ready", () => {
    ...
    log("[READY] Timestamp set to: " + readyTimestamp + " (only process messages after this)");
