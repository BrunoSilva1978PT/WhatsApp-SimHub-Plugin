...
namespace WhatsAppSimHubPlugin.Core
{
    ...
    public class MessageQueue
    {
        ...
        private int _timerResetCount = 0;                  // Contador de resets do timer
        private const int MAX_TIMER_RESETS = 3;            // Máximo de resets permitidos
        ...
        {
            // ✅ Limite POR CONTACTO (não limite global!)
            var contactMessages = _vipUrgentQueue
                .Where(m => m.Number == message.Number)
                .OrderBy(m => m.Timestamp)
                .ToList();
            
            if (contactMessages.Count >= _settings.MaxGroupSize)
            {
                // Remove a MAIS ANTIGA deste contacto
                var oldest = contactMessages.First();
                _vipUrgentQueue.Remove(oldest);
                
                _logger($"[QUEUE] Removed oldest message from {message.From} " +
                       $"(contact limit {_settings.MaxGroupSize} reached)");
            }
            ...
            _logger($"[QUEUE] Added {priority} message from {message.From} " +
                   $"(VIP/URGENT queue: {_vipUrgentQueue.Count})");
        }
        
        private void AddToNormalQueue(QueuedMessage message)
        {
            // ✅ Limite GLOBAL
            if (_normalQueue.Count >= _settings.MaxQueueSize)
            {
                // Remove a mais antiga (global)
                var oldest = _normalQueue.OrderBy(m => m.Timestamp).FirstOrDefault();
                if (oldest != null)
                {
                    _normalQueue.Remove(oldest);
                    _logger($"[QUEUE] Removed oldest NORMAL message (queue limit reached)");
                }
            }
            
            _normalQueue.Add(message);
            _logger($"[QUEUE] Added NORMAL message from {message.From} " +
                   $"(Normal queue: {_normalQueue.Count}/{_settings.MaxQueueSize})");
        }
        ...
        private void CheckForTimerReset(QueuedMessage newMessage)
        {
            ...
            if (_currentDisplayGroup != null && 
                ...
                _timerResetCount < MAX_TIMER_RESETS)
            {
                // ✅ RESET TIMER!
                _displayTimer.Stop();
                _timerResetCount++;
                
                _logger($"[TIMER] Reset {_timerResetCount}/{MAX_TIMER_RESETS} for {newMessage.From}");
